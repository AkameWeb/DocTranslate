...
async function loadHistoryFromServer() {
    ...
    // Загружаем истории конвертаций (если скрипты подключены)
    if (typeof loadConversionsHistory === 'function') {
        loadConversionsHistory();
    }
    if (typeof loadAudioConversionsHistory === 'function') {
    ...
    if (currentUser) {
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                // Принудительно загружаем свежую историю с сервера
                const freshRes = await fetch('get-history.php?t=' + Date.now(), { cache: 'no-store' });
                const freshData = await freshRes.json();
                appHistory = freshData;
                renderHistory();
            } else {
...
function renderHistoryActions() {
    ...
    let actionsHtml = '';
    if (currentUser) {
        actionsHtml = `<button class="clear-my-history" id="clearMyHistoryBtn"><i class="far fa-trash-alt"></i> Очистить</button>`;
    } else {
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Инициализация темы и кнопка переключения
